...
async def call_gemini_stream_with_retry(
    chat_session: genai.ChatSession, # Changed type hint here
    prompt: str,
    max_retries: int = 5,
    initial_delay: float = 1.0
) -> str:
    """
    Sends a message to the Gemini API and streams the response with exponential backoff.

    Args:
        chat_session (genai.ChatSession): The active chat session.
    ...
    Returns:
        str: The concatenated text from the streamed model response.
    ...
    for i in range(max_retries):
        try:
            ...
            print(f"Attempt {i+1}/{max_retries} failed to get stream response: {e}")
            if i < max_retries - 1:
                time.sleep(delay)
